...
class Tweet extends User{
	...
	public function tweetsHome($user_id, $num){
	    ...
	    foreach ($tweets as $tweet) {
		  ...
		  $comment = $this->comments($user_id, $tweet->tweetID);
			        ...
			        <div class="t-s-b-inner">
			      	...
			      	<div class="t-show-footer">
			      		<div class="t-s-f-right">
			      			<ul>	
			      				<li>'.((isset($comment['commentOn']) ? $tweet->tweetID === $comment['CommentOn']: '') ? 
									'<button class="retweet" data-tweet="'.$tweet->tweetID.'" data-user="'.$tweet->tweetBy.'" style="outline:none;"><i class="fa fa-comment" aria-hidden="true"></i></button>':
									'<button class="retweet" data-tweet="'.$tweet->tweetID.'" data-user="'.$tweet->tweetBy.'" style="outline:none;"><i class="fa fa-comment" aria-hidden="true"></i><span class="likesCounter">'.(($tweet->commentsCount > 0) ? $tweet->commentsCount : '' ).'</span></button>').'
								</li>
	...
	public function tweets($username, $num){
	    ...
	    $stmt = $this->pdo->prepare("SELECT * FROM `tweets` LEFT JOIN `users` ON `tweetBy` = `user_id` WHERE `tweetBy` = :user_id AND `retweetID` = '0' ORDER BY `tweetID` DESC LIMIT :num");
		$stmt->bindParam(":user_id", $user_id, PDO::PARAM_INT);
	    $stmt->bindParam(":num", $num, PDO::PARAM_INT);
	    $stmt->execute();
	    $tweets = $stmt->fetchAll(PDO::FETCH_OBJ);
	...
	public function addComment($user_id, $tweet_id, $get_id){
		$stmt = $this->pdo->prepare("UPDATE `tweets` SET `commentsCount` = `commentsCount`+1 WHERE `tweetID` = :tweet_id");
		$stmt->bindParam(":tweet_id", $tweet_id, PDO::PARAM_INT);
		$stmt->execute();

		$this->create('comments', array('commentBy' => $user_id, 'commentOn' => $tweet_id));
	...
	// public function comments($tweet_id, $user_id, $get_id, $comment){
	// 	$stmt = $this->pdo->prepare("UPDATE `tweets` SET `retweetCount` = `retweetCount`+1 WHERE `tweetID` = :tweet_id AND `tweetBy` = :get_id");
	// 	$stmt->bindParam(":tweet_id", $tweet_id, PDO::PARAM_INT);
	// 	$stmt->bindParam(":get_id", $get_id, PDO::PARAM_INT);
	// 	$stmt->execute();
